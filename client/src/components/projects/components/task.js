...
function Tasks(props) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = props.task.map((n) => n.tasks);
  ...
  const emptyRows =
    rowsPerPage - Math.min(rowsPerPage, props.task.length - page * rowsPerPage);
        ...
        <TableContainer>
          ...
          >
            <EnhancedTableHead
              ...
              rowCount={props.task.length}
            />
            <TableBody>
              {stableSort(props.task, getComparator(order, orderBy))
                ...
                .map((row, index) => {
                  ...
                  return (
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, row.tasks, index)}
                      role="checkbox"
...
class Task extends Component {
  ...
  handleTodayFilter = () => {
    this.props.handleTodayFilter();
  };

  handleWeekFilter = () => {
    this.props.handleWeekFilter();
  };

  handleMonthFilter = () => {
    this.props.handleMonthFilter();
  };
  ...
  handleRefreshTasks = () => {
    this.props.handleRefreshTasksFilter();
  };
  ...
  handleUpdateStatus = () => {
    ...
    data.forEach((res) => {
      ...
      const url_put = `https://${this.props.url}/project-task/${res.id}/`;
      ...
      axios
        ...
        .then((res) => {
          var data = this.props.task.filter((ss) => {
            return ss.id !== res.data.id;
          });
          this.props.handleEditstatustask(data, res.data);
        });
    });
    this.setState({
      loading: false,
