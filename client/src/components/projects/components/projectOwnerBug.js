      ...
      {numSelected > 0 ? (
        <>
          ...
          <Tooltip title="Delete">
            <IconButton
              aria-label="delete"
              onClick={() => props.handleDeletebugs(props)}
            >
              <DeleteIcon />
            </IconButton>
          </Tooltip>
        ...
        <>
          <Tooltip title="Refresh">
            <IconButton
              aria-label="Refresh"
              onClick={() => props.handleRefreshBugs()}
            >
              <RefreshIcon />
            </IconButton>
          </Tooltip>
...
function Bugs(props) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = props.data.map((n) => n.bugs);
    ...
    <div className={classes.root}>
      ...
      <div style={{ textAlignLast: "end" }}>
        ...
        >
          ...
          >
            <DialogTitle id="form-dialog-title">Assign Bug</DialogTitle>
          </div>
          <DialogContent>
            <TextField
              ...
              id="name"
              value={props.content}
              onChange={(event) => props.handlebugcontent(event)}
              label="Bug"
              type="text"
              autoComplete="off"
              fullWidth
      ...
      >
        <EnhancedTableToolbar
          ...
          handleRefreshBugs={props.handleRefreshBugs}
          search={props.search}
          ...
          handleEditbugs={props.handleEditbugs}
          handleDeletebugs={props.handleDeletebugs}
        />
        <TableContainer>
          <Table
...
class BugOwner extends Component {
  state = {
    selected: [],
    search: "",
    project_member: [],
  ...
  handleDeletebugs = (props) => {
    ...
    select.forEach((res) => {
      data = data.concat(
        this.props.bugFull.filter((filt) => {
    ...
    data.forEach((res) => {
      ...
      axios
        .delete(`https://${this.props.url}/project-bugs-owner/${res.id}`)
        .then((res1) => {
          this.props.handleDeleteBugsOwner(res.id);
        });
    });
    this.setState({
      loading: false,
  ...
  handleTodayFilter = () => {
    this.props.handleTodayFilterBugsOwner();
  };

  handleWeekFilter = () => {
    this.props.handleWeekFilterBugsOwner();
  };

  handleMonthFilter = () => {
    this.props.handleMonthFilterBugsOwner();
  };
  ...
  handleChange = (event) => {
    ...
    this.props.handleSearchBugsOwner(event.target.value);
  };

  handleRefreshBugs = () => {
    this.props.handleRefreshBugsOwnerFull();
  };
  ...
  handleAssignData = (event) => {
    ...
    form_data.append("bugs", this.state.content);
    ...
    const url_post = `https://${this.props.url}/project-bugs/`;
    axios.defaults.headers = {
      "Content-Type": "application.json",
      Authorization: "Token " + this.props.token,
    };
    axios
      ...
      .then((res) => {
        this.props.handleCreateBugsOwner(res.data);
        this.setState({
          loading: false,
        });
      });
  ...
  handleUpdateStatus = () => {
    ...
    data.forEach((res) => {
      ...
      const url_put = `https://${this.props.url}/project-bugs-owner/${res.id}/`;
      axios.defaults.headers = {
        "Content-Type": "application.json",
        Authorization: "Token " + this.props.token,
      };
      axios
        ...
        .then((res) => {
          this.props.handleUpdateBugsOwner(res.data);
        });
    });
    this.setState({
      loading: false,
  ...
  render() {
    return (
      <Bugs
        ...
        handlebugcontent={this.handlebugcontent}
        handleStatus={this.handleStatus}
        project_member={this.state.project_member}
        project={this.props.data}
        handleRefreshBugs={this.handleRefreshBugs}
        ...
        handleEditbugs={this.handleEditbugs}
        handleDeletebugs={this.handleDeletebugs}
...
export default connect(mapStateToProps)(BugOwner);
