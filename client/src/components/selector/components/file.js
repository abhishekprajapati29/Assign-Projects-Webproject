...
export default function TransferList(props) {
  ...
  const handleCheckedRight = () => {
    ...
    props.handleCheckedRightFile(checked)
  };

  const handleCheckedLeft = () => {
   
    setChecked(not(checked, rightChecked));
    props.handleCheckedLeftFile(checked);
  };
        ...
        {items.map((value) => {
          ...
          return (
            <ListItem key={value.id} role="listitem" button onClick={handleToggle(value)}>
              <ListItemIcon>
              <img alt="doc" src="https://img.icons8.com/color/24/000000/documents.png"/>
              </ListItemIcon>
              {
                (value.size)?
                (
                  <ListItemText id={labelId} primary={`${value.title} (${formatBytes(value.size)})`} />
                ):
